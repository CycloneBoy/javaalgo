...
public class Graph {
  ...
  public static boolean processRequest(Scanner in, Graph g) {
    try {
      ...
      if (alg.equals("u")) g.unweighted(startName);
      else if (alg.equals("d")) {
        g.dijkstra(startName);
        g.printPath(destName);
        g.dijkstra2(startName);
      } else if (alg.equals("n")) g.negative(startName);
      else if (alg.equals("a")) g.acyclic(startName);
  ...
  /**
   * A main routine that: 1. Reads a file containing edges (supplied as a command-line parameter);
   * 2. Forms the graph; 3. Repeatedly prompts for two vertices and runs the shortest path
   * algorithm. The data file is a sequence of lines of the format source destination cost
   */
  ...
  /**
   * Driver routine to handle unreachables and print total cost. It calls recursive routine to print
   * shortest path to destNode after a shortest path algorithm has run.
   */
  public void printPath(String destName) {
    Vertex w = vertexMap.get(destName);
    if (w == null) throw new NoSuchElementException("Destination vertex not found");
    else if (w.dist == INFINITY) System.out.println(destName + " is unreachable");
    else {
  ...
  /**
   * Recursive routine to print shortest path to dest after running shortest path algorithm. The
  ...
  public void acyclic(String startName) {
    ...
    for (Vertex v : vertexSet) for (Edge e : v.adj) e.dest.scratch++;

    // Enqueue vertices of indegree zero
    for (Vertex v : vertexSet) if (v.scratch == 0) q.add(v);
